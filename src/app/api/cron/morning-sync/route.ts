...
export async function GET(request: NextRequest) {
  ...
  if (!settingsList?.length) {
    return NextResponse.json({ ok: true, message: "No users with Google Calendar connected", synced: 0 });
  }

  const now = new Date();

  // timeMin = začátek dnešního dne v UTC (ne včera, ne přesně „teď")
  ...
  for (const row of settingsList as {
    ...
    try {
      // Smazat všechny prohlídky od začátku dnešního dne – budou znovu načteny
      await supabaseAdmin
        .from("viewings")
        .delete()
        .eq("user_id", row.user_id)
        .gte("event_start", timeMin)
      ...
      for (const ev of items) {
        ...
        let parsed = parseCalendarEvent(ev.id, summary, description, location, start, end ?? start, keyword);
      ...
      if (missingPhoneItems.length > 0) {
        const list = missingPhoneItems
          ...
          .join("\n");
        await notify(row, `⚠️ Chybí tel. číslo u ${missingPhoneItems.length} prohlídky`, `⚠️ Prohlídky bez telefonního čísla:\n${list}\n\nDoplňte číslo v Google Kalendáři nebo prohlídku zrušte.`).catch(() => {});
      }
      if (suspiciousNameItems.length > 0) {
        const list = suspiciousNameItems
          ...
          .join("\n");
        await notify(row, `⚠️ Zkontrolujte jména klientů (${suspiciousNameItems.length})`, `⚠️ U těchto prohlídek vypadá jméno klienta podezřele:\n\n${list}`).catch(() => {});
      }
    } catch (err) {
      console.error(`Morning sync for user ${row.user_id}:`, err);
    }
  }

  return NextResponse.json({ ok: true, synced: totalSynced, users: settingsList.length });
}
